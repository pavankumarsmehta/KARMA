...
$(function () {
	...
	$('body').on('click', '.btnSendStatusEmailToClient', function (e) {
		e.stopPropagation();
		e.preventDefault();
		$('#actType').val('SendStatusMail');;
		$('#frmOrderDetails').submit();
	});
	...
	// 	e.stopPropagation();
	// 	e.preventDefault();
	// 	let orderStatusVal = $(this).val();
	// 	SendStatusEmailToClient(orderStatusVal);
	// });	
	let orderStatusVal = $('#order_status').val();
	SendStatusEmailToClient(orderStatusVal);	
	
});

let SendStatusEmailToClient = function(orderStatusVal = '') {
	if(jQuery.inArray(orderStatusVal, ['Completed','Canceled'])!== -1) {	
		$('.btnSendStatusEmailToClient-js').removeClass('hidden').addClass('btnSendStatusEmailToClient');
	}else{
		$('.btnSendStatusEmailToClient-js').addClass('hidden').removeClass('btnSendStatusEmailToClient');
	}
}
