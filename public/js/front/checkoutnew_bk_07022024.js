		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.CouponDiscount == 0) {
					if (coupon_code != '') {
						$("#coupon_alert").css("color", "#ff0000").show();
					}
				} else {
					$("#coupon").val(coupon_code).attr("readonly", "readonly");
					$("#coupon_apply").hide();
					$("#coupon_remove").show();
					$("#coupon_alert").html("Coupon applied successfully, Got discount of $" + data.CouponDiscount).css("color", "#302b7b").show();
				}
				Ajax_GetOrder_Summery();
				$("#page-spinner").hide();
			}
		...
		if (confirm("Are you sure, you want to remove coupon code?")) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					$("#coupon_remove").hide();
					$("#coupon_apply").show();
					$("#coupon_alert").html("Coupon removed successfully").css("color", "#302b7b").show();
					$("#page-spinner").hide();
					Ajax_GetOrder_Summery();
				}
			})
