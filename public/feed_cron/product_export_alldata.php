...
function Get_Product_Category($products_id)
{
	...
	$catres = $obj->select("SELECT category_id FROM ".TABLE_PREFIX."products_category
	 						WHERE products_id = '".$products_id."' ORDER BY category_id"
						   );
...
function Get_Category_Structure($category_id)
{
    ...
    if(!empty($category_id) or $$category_id != '')
    {
        $db_sql = $obj->select("SELECT category_id,parent_id, category_name
                                FROM ".TABLE_PREFIX."category WHERE category_id = '".$category_id."'"
                               );
...
for ($i = 0; $i < $totalprodcount; $i++) 
 {
	...
	foreach($gen_csv_fields_arr as $gen_csv_fields_arr_key => $gen_csv_fields_arr_value){
		if(isset($result[$i][$gen_csv_fields_arr_value['export_field']])){
			...
			/* $product_category = '';
	         $category_res = Get_Product_Category($result[$i]["product_id"]);
...
//die;
$filename = "export_products_new" . time() . ".csv";
