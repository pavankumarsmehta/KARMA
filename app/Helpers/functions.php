...
use App\Http\Controllers\CategoryController;
	...
	/* brands logo aren't showing up  categories wise, also the shop by brand for each category. Can you check? 18-04-2024 Start  */
	/* if (!function_exists('BrandsList')) {
	...
	/* brands logo aren't showing up  categories wise, also the shop by brand for each category. Can you check? 18-04-2024 End  */
	if (!function_exists('BrandsList')) {
		...
		function BrandsList($BrandChar='',$category_id='',$category_name='')
		{
			...
			if(!empty($category_id) && !empty($category_name)){
				...
				$fetchvalue = getChildCatIdArray($category_id);
				if (!empty($fetchvalue)) {
			...
			if($BrandChar != '')
			{
				...
				if(isset($brandObj) && !empty($brandObj)){
					...
					$filterBrandArr = $brandObj->filter(function ($brand, $key) use($BrandChar,$category_id,$allcat) {
					if($BrandChar == '#'){
							if(!empty($category_id)){
								//if($brand->category_id==$category_id){
			...
			foreach (range('A', 'Z') as $char){
				...
				$filterBrandArr = array();
				if(isset($brandObj) && !empty($brandObj)){
					$filterBrandArr = $brandObj->filter(function ($brand, $key) use($char,$category_id,$allcat) {
						if(!empty($category_id)){
							//if($brand->category_id==$category_id){
							if(in_array($brand->category_id,$allcat)){
								$char1 = $char.'%';
			...
			$filterBrandArr = array();
			if(isset($brandObj) && !empty($brandObj)){
					$numberFilterBrandArr = $brandObj->filter(function ($brand, $key) use($category_id,$allcat)  {
						if(!empty($category_id)){
							//if($brand->category_id==$category_id){
							if(in_array($brand->category_id,$allcat)){
								return 1 === preg_match('/^[0-9]+/', strtolower($brand->brand_name));
		...
		function getAllBrand() {
			try {
				if (Cache::has('getAllBrandProductWise_cache')){
					...
					->select('po.product_id','po.product_name','b.brand_id','b.brand_name','b.brand_description','b.brand_logo_image','bc.category_id','b.display_on_category','b.brand_logo_image','b.display_position')
					->where('po.status','=','1')
					->where('b.status','=','1')
					->orderBy('b.brand_name')->get();
...
if (!function_exists('getChildCatIdArray')) {
	function getChildCatIdArray($category_id){
		$data = new ProductController();
		$getChildCatIdArray = $data->getChildCatIdArray($category_id);
		 return $getChildCatIdArray;
	}
}
