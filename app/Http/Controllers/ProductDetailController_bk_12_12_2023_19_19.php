...
class ProductDetailController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		$PackSizeWiseProductsSelected = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$product_size) {
			if($product->pack_size != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1  && $product->size==$product_size){
				return true;
			}else{	
				return false;
			}
		...
		$FlavourWiseProductsSelected = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$product_size,$product_pack_size) {
			if($product->flavour != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 && $product->size==$product_size){
				return true;
			}else{	
				return false;
			}
	...
	public function GetProductDetailView(Request $request)
	{
		...
		$getCurrentProductObj = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$CheckProductVariantAcitveSizeValue,$CheckProductVariantAcitvePackSizeValue,$CheckProductVariantAcitveFlavourValue) {
			if($product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 && $product->size == $CheckProductVariantAcitveSizeValue && $product->pack_size == $CheckProductVariantAcitvePackSizeValue && $product->flavour == $CheckProductVariantAcitveFlavourValue){
				return true;
			}else{
				return false;
			}
		...
		$SizeWiseProductsSelected = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$currentSelectedVariantName,$currentSelectedValue,$size_check,$CheckProductVariantAcitveSizeValue, $CheckProductVariantAcitvePackSizeValue, $CheckProductVariantAcitveFlavourValue) {
			if(!$size_check){
				if($currentSelectedVariantName=='flavour'){

					if($product->size != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 && $product->$currentSelectedVariantName==$currentSelectedValue){
						return true;
					}else{
						return false;
					}

				}elseif($currentSelectedVariantName=='pack_size'){
					if($product->size != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 && $product->$currentSelectedVariantName==$currentSelectedValue){
						return true;
					}else{
						return false;
		...
		$PackSizeWiseProductsSelected = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$currentSelectedVariantName,$currentSelectedValue,$pack_size_check,$CheckProductVariantAcitveSizeValue, $CheckProductVariantAcitvePackSizeValue, $CheckProductVariantAcitveFlavourValue) {
			if(!$pack_size_check){
				if($currentSelectedVariantName=='flavour'){
				//	dd('tet');
					if($product->pack_size != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 &&$product->$currentSelectedVariantName==$currentSelectedValue && $product->size==$CheckProductVariantAcitveSizeValue){
						return true;
					}else{
						return false;
					}
				}elseif($currentSelectedVariantName=='size'){
						//dd('tet');
						if($product->pack_size != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 &&$product->$currentSelectedVariantName==$currentSelectedValue){
							return true;
						}else{
							return false;
						}
		...
		$FlavourWiseProductsSelected = $ProductsObj->filter(function ($product, $key) use($product_group_code,$status_val,$currentSelectedVariantName,$currentSelectedValue,$flavour_check,$CheckProductVariantAcitveSizeValue, $CheckProductVariantAcitvePackSizeValue, $CheckProductVariantAcitveFlavourValue) {
			if(!$flavour_check){
				if($currentSelectedVariantName=='size'){
					...
					//dd($product->flavour != '' , $product->product_group_code , $product_group_code , $product->status,$status_val , $product->category_status,1 ,$product->$currentSelectedVariantName,$currentSelectedValue ,$product->pack_size,$CheckProductVariantAcitvePackSizeValue);
					if($product->flavour != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 &&$product->$currentSelectedVariantName==$currentSelectedValue && $product->pack_size==$CheckProductVariantAcitvePackSizeValue){
					...
					if($product->flavour != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 &&$product->$currentSelectedVariantName==$currentSelectedValue && $product->size==$CheckProductVariantAcitveSizeValue){
						return true;
					}else{
						return false;
					}
				}else{
					if($product->flavour != '' && $product->product_group_code == $product_group_code && $product->status==$status_val && $product->category_status==1 &&$product->$currentSelectedVariantName==$currentSelectedValue){
						return true;
					}else{
						return false;
					}
	...
	//	dd($getCurrentProductObj);
		...
		$this->PageData['Wishproducts_id'] =  $Wishproducts_id;
		$productdetail_right_variant_size =  $productdetail_right_color = view('product_detail.details.productdetail_right_variant_size')->with($this->PageData)->render();
		$productdetail_right_variant_pack_size =  $productdetail_right_color = view('product_detail.details.productdetail_right_variant_pack_size')->with($this->PageData)->render();
		$productdetail_right_variant_flavour =  $productdetail_right_color = view('product_detail.details.productdetail_right_variant_flavour')->with($this->PageData)->render();
		$productdetail_right_desc_section =  $productdetail_right_color = view('product_detail.details.productdetail_right_desc')->with($this->PageData)->render();
		$product_left_image_section =  $productdetail_right_color = view('product_detail.details.productdetail_leftimage')->with($this->PageData)->render();
		...
		//dd($getCurrentProductObj,$FlavourWiseProductsSelected_arr,$PackSizeWiseProductsSelected_arr);
		//dd($FlavourWiseProducts_arr);
		//dd($PackSizeWiseProductsSelected_arr);
		//dd($PackSizeWiseProducts_arr);
