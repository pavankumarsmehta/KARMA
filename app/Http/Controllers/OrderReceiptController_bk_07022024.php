...
class OrderReceiptController extends Controller
{
	...
	public function ProductDeductStock($product_sku, $quantity)
	{
		$ProductInfo = DB::table($this->prefix.'products')
			->select('quantity','sold_qunantity')
		...
		if(!empty($ProductInfo))
		{
			...
			$new_stock = $ProductInfo->quantity - $quantity;
	...
	public function sendOrderEmail($orderID, $customerID)
	{
		...
		echo $EmailBody; //exit;
		//echo config('Settings.CONTACT_MAIL');
		
		//echo $EmailBody; exit;
		/*$From = 'cs@hbasales.com';
		## Send Email TO Customer
		##------------------------
		//$a = $this->sendSMTPMail('cs@hbasales.com', $Subject, $EmailBody, $From );
		$a = $this->sendSMTPMail_Normal($ToEmail, $Subject, $EmailBody, $From );
		//$a = $this->sendSMTPMail('sachin.qualdev@gmail.com', $Subject, $EmailBody, $From );
		$headers  = "From: " . strip_tags($From) . "\r\n";
		$headers .= "Reply-To: " . strip_tags($From) . "\r\n";
		$headers .= "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		//$a = $this->sendSMTPMail_Normal($ToEmail, $Subject, $EmailBody, $headers );
		//$a = $this->sendSMTPMail($ToEmail, $Subject, $EmailBody, $From );*/
		
		
		//echo $a."==========".$EmailBody; exit;
		## Send Email TO Admin 	
		##----------------------
		//$b = $this->sendSMTPMail(config('const.ADMIN_EMAIL'), $Subject, $EmailBody, 'orders@hbasales.com');
		//$b = $this->sendSMTPMail_Normal('sachin.qualdev@gmail.com', $Subject, $EmailBody, $headers);
