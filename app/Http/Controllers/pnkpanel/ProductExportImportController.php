...
class ProductExportImportController extends Controller
{
    ...
    public function export(Request $request)
    {
     ...
     public function updateexportproduct_view()
    {
        $product = new Product;
        $manufacture = Manufacturer::orderBy('manufacturer_name', 'ASC')->get();
        $pageData['page_title'] = "Export Stock/Price";
        $pageData['meta_title'] = "Export Stock/Price";
        $pageData['breadcrumbs'] = [
            [
                'title' => 'Export Stock/Price',
                'url' => route('pnkpanel.product.updateexportproduct_view')
            ]
        ];
        return view('pnkpanel.export_products.limited_export_product_view', compact('product', 'manufacture'))->with($pageData);
    }
    ...
    public function import(Request $request)
    {
        ...
        if (File::exists($file_path)) {
            ...
            while ($data = fgetcsv($handle, $getfilesizevar, ",",'"','\\')) {
                ...
                for ($tc = 0; $tc < (count($first_header_rows_arr)-1); $tc++) {
                    ...
                    if (array_key_exists($first_header_rows_arr[$tc], $gen_csv_fields_arr)) {
                        ...
                        if($tc==36)
                        {
                            //echo $import_field_name; exit;
                        }
                        if ($import_field_name == 'product_name' || $import_field_name == 'ingredients' || $import_field_name == 'ingredients_pdf' || $import_field_name == 'uses' || $import_field_name == 'key_features' || $import_field_name == 'short_description' || $import_field_name == 'product_description' || $import_field_name == 'meta_title' || $import_field_name == 'meta_keyword' || $import_field_name == 'meta_description' ) {
                            $str = stripslashes($data[$tc]);
                            $str = utf8_encode($str);
                            $str = str_replace("", "'", $str);
                            $fieldvalue = trim($str);
                        } else {
                            //echo $data[$tc]."<br>++++<br>"; //exit;
                            $fieldvalue = trim(stripslashes($data[$tc]));
                            //$fieldvalue = trim(addslashes($data[$tc]));
                        }
                        //echo $fieldvalue."<br>+++++++<br>"; //exit;
                        if ($import_field_name != '') {
                ...
                if(isset($insert_imp_arr['on_sale'])){
                  
                    if(strtolower($insert_imp_arr['on_sale'])=='yes'){
                        if($insert_imp_arr['sale_price'] == '0'){
                            $insert_imp_arr['status']= '0';
                            
                        }
                        
                    }else{
                        if($insert_imp_arr['our_price'] == '0'){
                            $insert_imp_arr['status']= '0';
                        }
                        
                        if($insert_imp_arr['our_price'] == '0.00'){
                            $insert_imp_arr['status']= '0';
                        }
                    }
                    
                    if(strtolower($insert_imp_arr['on_sale'])=='yes'){
                        if($insert_imp_arr['sale_price'] == '0.00'){
                            $insert_imp_arr['status']= '0';
                        }
    ...
    public function import_limited_batch(Request $request)
    {
        global     $err_fp;
        //echo "11"; exit;
        $tablename = "hba_import_limited_products";
        $gen_csv_fields_arr = config('limitimportproduct');
        if ($request->start_limit != '') {
            $start_limit = $request->start_limit;
        } else {
            $start_limit = 0;
        ...
        if ($start_limit == 0) {
            ...
            $show_error_report = 0;
            ## Generated error report during processing products_new start
            $error_report_file_path = config('const.LIMITED_IMPORT_CSV_PATH') . "Error_Report.csv";
            if (File::exists($error_report_file_path)) {
                File::delete($error_report_file_path);
            }
            ...
            $csv_fields_arr = $request->session()->get('sess_limited_first_header_row_arr');
        ...
        Log::info(print_r("total_record = " . $total_record, true));
        if ($start_limit <= $total_record) {
            $csv_fields = $request->session()->get('sess_limited_import_csv_field');
            $result = DB::table($tablename)->select('*')->where('sku', '<>', "SKU")->orderBy('import_limited_product_id')->offset($start_limit)->limit($end_limit)->get()->toArray();
           ...
           $error_report_file_path =  config('const.LIMITED_IMPORT_CSV_PATH') . "Error_Report.csv";
            ...
            if (isset($result) && count($result) > 0) {
                $this->ProductDataExecutionLimited($result, $request);
                // sleep(2);
            }

            fclose($err_fp);
            return view("pnkpanel.import_products.limited_import_process", compact('start_limit', 'end_limit', 'show_error_report', 'total_record'));
        } else {
          ...
          //  $from = "cs@hbasales.com";
           // $to = 'sachin.qualdev@gmail.com';
            ...
            $request->session()->forget('sess_import_csv_field');
            $request->session()->forget('sess_first_header_row_arr');
            return redirect()->route('pnkpanel.product.updateimportproduct')->with(['message' => "Products Imported Successfully."]);
        }
    }
    ...
    function ProductDataExecution($result, $request)
    {
        ...
        for ($i = 0; $i < count($result); $i++) {
                       ...
                       if ($product_field_name != '') 
                           {
                            ...
                            elseif ($product_field_name == "product_description") {
                                ...
                                $imp_data_arr[$product_field_name] = $fieldvalue;
                            } 
            ...
            if (!$error_flag) {
                ...
                $imp_data_arr['product_group_code'] = ltrim($imp_data_arr['product_group_code'],"H-");
                $imp_data_arr['product_group_code'] = ltrim($imp_data_arr['product_group_code'],"H-H");
                if (count($checkProduct) > 0) {
                    ...
                    $imp_data_arr['status'] = $imp_data_arr['status'];
                    $product_id = intval($checkProduct[0]["product_id"]);
                    /****************************************************/
                   // echo $imp_data_arr['status'] ; exit;
    ...
    function ProductDataExecutionLimited($result, $request)
    {
       
        global $gen_csv_fields_arr, $gen_required_fields, $show_error_report, $err_fp, $manufacture_id;
        $gen_csv_fields_arr = config('limitimportproduct');
        $gen_required_fields = config('requiredfields.gen_required_limited_import_fields');
        $csv_fields_arr         = array();
        $csv_fields_arr         =  $request->session()->get('sess_limited_first_header_row_arr');
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            if ($error_flag) {
                ...
                $err_report_data_row .= "\n";
                $show_error_report = 1;
                return redirect()->route('pnkpanel.product.updateimportproduct')->with('error', $err_report_data_row);
            }
    ...
    public function updateimportproduct_view(Request $request){
       
        $pageData['page_title'] = "Import Stock/Price";
        $pageData['meta_title'] = "Import Stock/Price";
        $pageData['breadcrumbs'] = [
            [
                'title' => 'Import Stock/Price',
                'url' => route('pnkpanel.product.updateimportproduct')
            ]
        ];
        return view('pnkpanel.import_products.limited_import_product_view')->with($pageData);
    }
    public function updateimportproduct(Request $request)
    {
        ...
        $filename = $request->file('import_product_file')->getClientOriginalName();
        $file_path = config('const.LIMITED_IMPORT_CSV_PATH') . $filename;
        ...
        $file = $request->file('import_product_file');
        $file->move(config('const.LIMITED_IMPORT_CSV_PATH'), $filename);
        ...
        if (File::exists($file_path)) {
            ...
            while ($data = fgetcsv($handle, $getfilesizevar, ",",'"','\\')) {
                ...
                if ($rec_counter == 1) {
                    ...
                    foreach ($data as $field_num => $field_value) {
                        ...
                        if (!array_key_exists($col_header_name, $gen_csv_fields_arr) && !in_array($col_header_name, array())) {
                            ...
                            $err_msg .= "Please follow the sample csv format.";
                            // $err_msg = rawurlencode($err_msg);
                            return redirect()->route('pnkpanel.product.updateimportproduct')->with('error', $err_msg);
                        }
                    ...
                    $field_list = str_replace("`", "", $field_list);
                    $request->session()->put('sess_limited_import_csv_field', $field_list);
                }
                ...
                $request->session()->put('sess_limited_first_header_row_arr', $first_header_rows_arr);
                ...
                for ($tc = 0; $tc < (count($first_header_rows_arr)-1); $tc++) {
                    ...
                    if (array_key_exists($first_header_rows_arr[$tc], $gen_csv_fields_arr)) {
                       ...
                       $import_field_name = $gen_csv_fields_arr[$first_header_rows_arr[$tc]]['import_field'];
                        ...
                        //echo $fieldvalue."<br>+++++++<br>"; //exit;
                        if ($import_field_name != '' && $import_field_name != $fieldvalue) {
                            $insert_imp_arr[trim($import_field_name)] = $fieldvalue;
                        }
                    } else {
            ...
            return redirect()->route('pnkpanel.product.post_import_limited');
        }
        ## If End file exits
    }
}
