...
trait CartTrait
{
	...
	{ 
		...
		if(!empty($check_deal_of_week))
		{
				...
				$temp_ary['Price']    		= $ProductPriceRs['our_price'];
				$temp_ary['Price_disp']    	= $ProductPriceRs['our_price_disp'];
		}
		elseif($temp_ary['sale_price'] > 0 && $temp_ary['on_sale'] == 'Yes')
		{
				$temp_ary['TotPrice']    	= NumberFormat($temp_ary['sale_price'] * $temp_ary['Qty']);
				$temp_ary['Price']    		= $ProductPriceRs['sale_price'];
				$temp_ary['Price_disp']    	= $ProductPriceRs['sale_price_disp'];
		}
		else
		{
				$temp_ary['TotPrice']    	= NumberFormat($temp_ary['our_price'] * $temp_ary['Qty']);
		...
		$temp_ary['attribute_1'] = $temp_ary['size_dimension'] = $temp_ary['attribute_2'] = $temp_ary['pack_size'] = $temp_ary['attribute_3'] = $temp_ary['flavour'] = '';
		if (isset($ProductRs->attribute_1) && $ProductRs->attribute_1 != "" && isset($ProductRs->size) && $ProductRs->size != "") {	
			$temp_ary['attribute_1']     = ucwords(strtolower(str_replace("_"," ",$ProductRs->attribute_1)));
			$temp_ary['size_dimension']     = $ProductRs->size;
		}		
		// Second condition correction
		if (isset($ProductRs->attribute_2) && $ProductRs->attribute_2 != "" && isset($ProductRs->pack_size) && $ProductRs->pack_size != "") {
			$temp_ary['attribute_2'] = ucwords(strtolower(str_replace("_"," ",$ProductRs->attribute_2)));
			$temp_ary['pack_size'] = $ProductRs->pack_size;
		}
		// Third condition correction
		if (isset($ProductRs->attribute_3) && $ProductRs->attribute_3 != "" && isset($ProductRs->flavour) && $ProductRs->flavour != "") {
			$temp_ary['attribute_3'] = ucwords(strtolower(str_replace("_"," ",$ProductRs->attribute_3)));
			$temp_ary['flavour'] = $ProductRs->flavour;
		}
