...
class CheckoutController extends Controller
{
	...
	public function getPaymentMethods(Request $request)
	{
		...
		if(!empty($PaymentMethodList))
		{
			for($p=0; $p<count($PaymentMethodList); $p++)
			{
				...
				$ROW_STR .= '
				<div class="checkout-acd '.$cls_pm_display.'">
					<label class="title">
						<input type="radio" name="paymentMethod" id="paymentMethod'.$p.'" value="'.$PaymentMethodList[$p]->pm_group_name.'" '.$p_checked.' data-pmname="'.$pmname.'" class="check-input-rd">
						<span class="pm-logo"><img src="'.config('const.SITE_URL').'/images/check_credit_card.png" alt="Credit Card" /></span>
						'.$PaymentMethodList[$p]->pm_name.'
							...
							<div class="inner">
								<div class="row">
									<div class="col-sm-6 mb-3">
										...
										<div class="invalid-feedback">Please enter name on the card</div>
									</div>
								...
								<div class="row">
									<div class="col-sm-6 mb-3">
										...
										<div class="invalid-feedback">Please enter credit card number</div>
									</div>
	...
	public function getShippingMethods(Request $request)
	{
		...
		foreach($ShippingModeRS as $ShippingMode )
		{
			$ROW_STR .='<div class="ship-time">
							...
							<input class="form-check-input check-input-rd" type="radio" name="shippingModeId" id="shippingModeId'.$temp_count.'" value="'.$ShippingMode->shipping_mode_id.'" onclick="Ajax_GetOrder_Summery();" data-charge="'.$tempCharge.'" data-shipname="'.utf8_decode($ShippingMode->shipping_title).'" type="radio" checked="checked">
							<label class="form-check-label" for="shippingModeId'.$temp_count.'"> <strong>'.utf8_decode($ShippingMode->shipping_title).'</strong></label>
						</div>
					</label>';
	...
	public function OrderConfirm(Request $request)
	{
		...
		$temp_ary['parent_sku']         = 'ABC123';
		$temp_ary['parent_category_id'] = '1';
		$temp_ary['size_dimension']     = '30 ML';
		$temp_ary['shipping_text']      = 'Shipped Tomorrow';
		$temp_ary['shipping_days']      = '1 Days';
		$temp_ary['upc']         		= 'UPC45457815215';
