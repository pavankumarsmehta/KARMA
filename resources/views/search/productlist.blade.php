...
<div class="container">
	...
	<div class="list_hd"><h2 tabindex="0">@if(!empty($PageTitle)){{$PageTitle}}@endif</h2></div>
	<div class="toolbar">
		@if(count($Products) > 0)	
		...
		<div class="toolbar-right">
			...
			<div class="form-floating">
				<label class="hidden-xs-down" for="sortopt" tabindex="0">Sort By:</label>
				<select class="form-select" id="sortopt" tabindex="0" aria-label="Floating label select example">
					<option {{ $SelSort=="" ? 'selected' : '' }} value="">Select Sorting</option>
					<option {{ $SelSort=="PLTH" ? 'selected' : '' }} value="PLTH">Price Low to High</option>
					<option {{ $SelSort=="PHTL" ? 'selected' : '' }} value="PHTL">Price High to Low</option>
					<option {{ $SelSort=="AZ" ? 'selected' : '' }} value="AZ">A-Z</option>
					<option {{ $SelSort=="ZA" ? 'selected' : '' }} value="ZA">Z-A</option>
					<option {{ $SelSort=="newest" ? 'selected' : '' }} value="newest">Newest</option>
				</select>
			</div>
		</div>
		@endif
			...
			<div class="row row-no-gutters">
				...
				<div class="col-xs-12 pt-5">
					<ul class="live_view" id="product_listing" data-load='0'>
						@if(count($Products) > 0)
                            <x-searchproductbox :prodData="$Products" :pageType="$pageType"  :sectionName="$sectionName"   />
						@endif
					</ul>
				</div>
