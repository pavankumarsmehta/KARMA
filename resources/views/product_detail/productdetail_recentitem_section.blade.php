	...
	<div class="gallery-slider clearfix">
		<div class="cm-hd">
			<div class="cm-hd-left">
				<div class="h5">Recently Viewed</div>
			</div>
		</div>
		<?php //dd($recent_item);?>
		<div class="slick-scroll slick-space-20">
			@for($i=0;$i<count($recent_item);$i++)
			<div class="product">
				<div class="product_thumb">
					<a href="{{$APP_URLS.$recent_item[$i]['product_url']}}" title="{{$recent_item[$i]['product_name_hover']}}">
						<picture><img src="{{$recent_item[$i]['product_thumb_image']}}" alt="" title="" width="365" height="365" loading="lazy" /></picture>
					</a>
					<a href="javascript:void(0)" data-productid="{{$recent_item[$i]['product_id']}}" rel="nofollow" title="Add to wishlist" class="displaypopupboxwishlist wishlist">
				...
				<div class="product_sku">{{strip_tags($recent_item[$i]['product_description'])}}</div>
				<div class="product_name"><a href="{{$APP_URLS.$recent_item[$i]['product_url']}}" rel="nofollow" title="{{$recent_item[$i]['product_name_hover']}}">{{$recent_item[$i]['product_name']}}</a></div>
				<div class="product_price">{{$recent_item[$i]['our_price_disp']}}</div>
