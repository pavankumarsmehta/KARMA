...
<div class="container">
	...
	<div class="row row10">
		<div class="col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2 pb-sm-5 pb-0 mb-5">
			<div class="tac pb-3">
				<h1 tabindex="0" class="h2">Forgot Password</h1>
			</div>
			...
			@if (count($errors) > 0 || Session::has('success'))
				...
				<div class="error tac">{{ $errors->first('not_exist_email') }}</div>
				@endif
				@if($errors->has('email'))
				<div class="error tac">{{ $errors->first('email') }}</div>
				@endif
				@if(Session::has('success'))
				<div class="success tac">{{ Session::get('success') }}</div>
				@endif
			...
			<form method="post" id="formForgotPassword" name="formForgotPassword" class="needs-validation ">
				...
				<div class="pt-3 order-row row row10">
