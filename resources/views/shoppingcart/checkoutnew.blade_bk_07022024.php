...
<div class="checkout">
	<div class="container">
		...
		<div class="ck-step">
			<a href="#" title="Contact">Contact<svg class="svg_barrow" width="23" height="23" aria-hidden="true" role="img" loading="lazy"><use href="#svg_barrow" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg_barrow"></use></svg></a> 
			<a href="#" title="Shipping/Billing Information">Shipping/Billing Information<svg class="svg_barrow" width="23" height="23" aria-hidden="true" role="img" loading="lazy"><use href="#svg_barrow" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg_barrow"></use></svg></a> 
					...
					<input type="hidden" name="BRAINTREE_GOOGLE_MERCHANT_ID" id="BRAINTREE_GOOGLE_MERCHANT_ID" value="{{$bt_api_details->BRAINTREE_GOOGLE_MERCHANT_ID ?? ""}}" /> 
						<div class="checkout-acd active" id="shipping-acd">
							...
							<div id="shipping_form_section">
								...
								<div class="row row10">
									<div class="col-xs-12 col-sm-6 pt-3">
										<label for="sh_Addr1" class="form-label">Address 1</label>
									...
									<div class="col-xs-12 col-sm-6 pt-3">
										<label for="sh_Addr2" class="form-label">Address 2 <span class="red-color">*</span></label>
						...
						<div class="checkout-acd" id="billing-acd">
							...
							<div id="billing_form_section" class="dnone">
								<div id="billing_adddress_box" style="display:none;">
									...
									<div class="pt-3">
										<label for="bl_Addr1" class="form-label">Address 1</label>
									...
									<div class="pt-3">
										<label for="bl_Addr2" class="form-label">Address 2 <span class="red-color">*</span></label>
						...
						<div class="shipping-method checkout-acd">
                                ...
                                <div class="title">
                                    ...
                                    <div class="securetxt">
                                        <svg class="svg_method" aria-hidden="true" role="img" width="16" height="17" loading="lazy" onClick="edit_checkout_step('shipping-method');">
                                        <use href="#svg_method" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg_method"></use>
                                        </svg> Shipping Information</div>
                                </div>
				...
				<div class="ch-contact">
					<div class="ch-contact-inr">
						...
						<a href="malito:{{ config('Settings.CONTACT_MAIL') }}" title="Email Us" class="linksbb">Email Us</a>
					</div>
					<div class="ch-contact-inr">
			...
			<div class="checkout_right">
				...
				<div class="order_summary">
					...
					<div class="yr-item">
						...
						@if($CartLength > 0)
							<div id="all-product-remove">
								...
								@foreach($Cart as $key => $val)
									<li>
										<a href="{{$val['product_url']}}" title="{{ $val['ProductName'] }}">
						...
						<div class="dnone" id="all-product-add">
							<ol class="Cart-items">
								...
								<li>
								<div class="product">
									<a href="{{$val['product_url']}}" class="product_thumb" title="{{ $val['ProductName'] }}"><picture><img src="{{ $val['Image'] }}" alt="{{ $val['ProductName'] }}" title="{{ $val['ProductName'] }}" width="90" height="90" loading="lazy"></picture></a>
									<div class="product_name"><a href="{{$val['product_url']}}" class="name text_c1" title="{{ $val['ProductName'] }}">{{ $val['ProductName'] }}</a></div>
					...
					<div class="coupon-code coupon-code-checkout">
						<div class="h4">Add a coupon code</div>
					...
					<p class="f14 tac mt-5">By placing an order, you agree to the {{ config('const.SITE_NAME')}} <a href="{{ config('const.SITE_URL') }}/pages/terms-and-condition" title="Terms & Conditions">Terms & Conditions</a> and <a href="{{ config('const.SITE_URL') }}/pages/privacy-policy" title="Privacy Policy">Privacy Policy</a></p>
				</div>
			</div>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCYByo3O3PZ85Fng7YLW7q7J9zbmg5hFTI&callback=initAutocomplete&libraries=places&v=weekly" defer></script>
...
<script>
	 // This sample uses the Autocomplete widget to help the user select a
      ...
      function initAutocomplete() {
        // Create the street_number object, restricting the search predictions to
        // geographical location types.
		//var input = document.getElementById('sh_Addr1');
		//var input_ship = document.getElementById('bl_Addr1');
		
		
        street_number = new google.maps.places.Autocomplete(
          document.getElementById("sh_Addr1"),
          { types: ["geocode"] }
        );
		street_number_bill = new google.maps.places.Autocomplete(
          document.getElementById("bl_Addr1"),
          { types: ["geocode"] }
        );
        // Avoid paying for data that you don't need by restricting the set of
        // place fields that are returned to just the address components.
        street_number.setFields(["address_component"]);
        //street_number_ship.setFields(["address_component"]);
        // When the user selects an address from the drop-down, populate the
        // address fields in the form.
        street_number.addListener("place_changed", fillInAddress);
        street_number_bill.addListener("place_changed", fillInAddress_bill);
      }

      function fillInAddress() 
	  {
        // Get the place details from the autocomplete object.
        const place = street_number.getPlace();
		
		//var address_components = componentForm;
		for (var i = 0; i < place.address_components.length; i++) 
		{
			...
			if(addressType == 'street_number')
				document.getElementById('sh_Addr1').value = place.address_components[i][componentForm[addressType]];
				
			if(addressType == 'route')
				var sh_address2 = place.address_components[i][componentForm[addressType]];
				
			//if(document.getElementById('sh_Addr1').value != "" && sh_address2!="")
				//document.getElementById('sh_Addr1').value = document.getElementById('sh_Addr1').value+' '+sh_address2;
				
			if(addressType == 'locality')
				document.getElementById('sh_city').value = place.address_components[i][componentForm[addressType]];
		
			if(addressType == 'postal_code')
				document.getElementById('sh_zip').value = place.address_components[i][componentForm[addressType]];
		
			if(addressType == 'administrative_area_level_1')
				document.getElementById('sh_state').value = place.address_components[i][componentForm[addressType]];
	...
	function fillInAddress_bill() 
	  {
        // Get the place_bill details from the autocomplete object.
        const place_bill = street_number_bill.getPlace();

		//var address_components = componentForm;
		for (var i = 0; i < place_bill.address_components.length; i++) 
		{
			var addressType = place_bill.address_components[i].types[0];
			//console.log(place_bill.address_components[i][componentForm[addressType]]);
			//return false;
			if(addressType == 'street_number')
				document.getElementById('bl_Addr1').value = place_bill.address_components[i][componentForm[addressType]];
				
			if(addressType == 'route')
				var bl_address2 = place_bill.address_components[i][componentForm[addressType]];
				
			//if(document.getElementById('sh_Addr1').value != "" && sh_address2!="")
				//document.getElementById('sh_Addr1').value = document.getElementById('sh_Addr1').value+' '+sh_address2;
				
			if(addressType == 'locality')
				document.getElementById('bl_city').value = place_bill.address_components[i][componentForm[addressType]];
		
			if(addressType == 'postal_code')
				document.getElementById('bl_zip').value = place_bill.address_components[i][componentForm[addressType]];
		
			if(addressType == 'administrative_area_level_1')
				document.getElementById('bl_state').value = place_bill.address_components[i][componentForm[addressType]];
			
			
		}
		if(bl_address2!="")
		{
			document.getElementById('bl_Addr1').value = document.getElementById('bl_Addr1').value+' '+bl_address2; 
		}
	}
