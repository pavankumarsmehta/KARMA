			...
			<div class="cart_row">
				<div class="thumb"><a href="{{$APP_URLS.$val['product_url']}}" title="{{ $val['ProductName'] }}"><picture><img src="{{ $val['Image'] }}" width="175" height="175" alt="{{ $val['ProductName'] }}" loading="lazy"></picture></a>
					<div class="hidden-sm-up">
						<div class="qty-box mt-2">
							...
							<input type="text" aria-label="Quantity box" name="{{$key}}" id="prod_{{$key}}_mob" data-field="prod_{{$key}}" data-productid="{{ $val['product_id'] }}" class="form-control input-number prod_{{$key}}" value="{{ $val['Qty'] }}" min="1" max="10" readonly />
				...
				<div class="info">
					...
					<div class="msclinks mt-3">
						...
						Add to wishlist</a>
						<a href="javascript:void(0);" class="remove removeCartItem" data-index="{{ $key }}" title="Remove">
				...
				<div class="cart_price hidden-xs-down">
					...
					<div class="qty-box">
						...
						<input type="text" aria-label="Quantity box" name="{{$key}}" id="prod_{{$key}}_desk" data-field="prod_{{$key}}"  data-productid="{{ $val['product_id'] }}" class="form-control input-number prod_{{$key}}" value="{{ $val['Qty'] }}" min="1" max="10" readonly>
						<button type="button" class="right-qty btn-number" data-type="plus" data-field="prod_{{$key}}">+</button>
					</div>
					{{-- <div class="available">
